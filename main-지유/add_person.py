...
class addPerson():
            ...
            if detected_faces:
                try:
                    ...
                    if count == 1:
                        ...
                        temp_visualized_data = df(data={'face_id': [get_face_id], 'Time': ['0000-00-00 00-00-00']},columns=['face_id','Time'])
                        print('temp_visualized_data\n',temp_visualized_data)
                        temp_visualized_list = s.visualized_df.append(temp_visualized_data)
                        temp_visualized_list = temp_visualized_list[['face_id','Time']]
                        print('temp_visualized_list\n',temp_visualized_list)

                        temp_visualized_list.to_csv("./csvfile/visualized_log.csv",index=False)
    ...
    def delete_person_from_list(self,index):
        ...
        df2 = df1.drop(index)
        tmp = visualized_df.drop(index)
        print(df2)
        print(tmp)
        df2.to_csv("./csvfile/personlist.csv", index=False)
        tmp.to_csv("./csvfile/visualized_log.csv", index=False)
        s.upload_file('./csvfile/', 'csv')
